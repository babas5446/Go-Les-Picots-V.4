...
struct JiggingChoiceSheet: View {
    ...
    // États pour les sheets de technique
    @State private var showSlowPitchTechnique = false
    @State private var showFastPitchTechnique = false
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $showSlowPitchTechnique) {
                SlowPitchTechniqueSheet()
            }
            .sheet(isPresented: $showFastPitchTechnique) {
    ...
    private var headerSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("Le slow jigging déclenche à la descente par opportunisme ; le fast jigging déclenche à la montée par poursuite :")
                    .font(.subheadline)
    ...
    private var fastJiggingCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Bouton Technique d'animation
            Button {
                showFastPitchTechnique = true
            } label: {
                HStack(spacing: 8) {
                    Image(systemName: "book.fill")
                    Text("Technique d'animation")
                        .fontWeight(.semibold)
                }
        ...
        .onTapGesture(count: 2) {
            if !selectedTypes.contains(.fastJigging) && selectedTypes.count < 3 {
                selectedTypes.append(.fastJigging)
            }
            dismiss()
        }
    }
    ...
    private var slowJiggingCard: some View {
        ...
        .shadow(color: Color.black.opacity(0.1), radius: 4, x: 0, y: 2)
        .onTapGesture(count: 2) {
            if !selectedTypes.contains(.slowJigging) && selectedTypes.count < 3 {
                selectedTypes.append(.slowJigging)
            }
            dismiss()
        }
    }
...
struct JiggingChoiceSheet_Previews: PreviewProvider {
    static var previews: some View {
        JiggingChoiceSheet(selectedTypes: .constant([]))
    }
}
